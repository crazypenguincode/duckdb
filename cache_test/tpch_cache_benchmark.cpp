...
struct QueryResult {
    ...
    QueryResult(const string& hash, const string& data, size_t size, double time)
        : query_hash(hash), result_data(data), result_size(size), 
...
struct CacheEntry {
    ...
    CacheEntry(const string& hash, unique_ptr<QueryResult> res)
...
// ML增强缓存条目
struct MLCacheEntry : public CacheEntry {
    double access_frequency;
    vector<double> access_intervals; // 访问间隔时间
    double predicted_next_access;
    double cache_value_score;
    
    MLCacheEntry(const string& hash, unique_ptr<QueryResult> res)
        : CacheEntry(hash, std::move(res)), access_frequency(1.0),
          predicted_next_access(0.0), cache_value_score(0.5) {}
};
...
struct BenchmarkStats {
    ...
    // 查询类型统计
    map<string, size_t> query_type_hits;
    map<string, size_t> query_type_total;
    ...
    double GetCacheEfficiency() const {
        return cache_hits > 0 ? 
            (total_execution_time / cache_hits) / GetAvgResponseTime() : 0.0;
    }
};
...
private:
    string queries_dir;
    vector<pair<string, string>> queries; // <query_name, query_sql>
...
public:
    ...
    void LoadQueries() {
        ...
        for (int i = 1; i <= 22; i++) {
            string filename = "q" + (i < 10 ? "0" : "") + to_string(i) + ".sql";
            ...
            if (file.is_open()) {
                ...
                cout << "  ✅ 加载查询: " << filename << " (" << query_sql.length() << " 字符)" << endl;
            } else {
                cout << "  ❌ 无法加载: " << filepath << endl;
            }
        }
    ...
    string CleanQuery(const string& query) {
        ...
        while ((pos = cleaned.find("--", pos)) != string::npos) {
            ...
            if (end == string::npos) end = cleaned.length();
...
public:
    ...
    virtual unique_ptr<QueryResult> ExecuteQuery(const string& query_name, const string& query_sql) = 0;
...
public:
    ...
    unique_ptr<QueryResult> ExecuteQuery(const string& query_name, const string& query_sql) override {
        stats.total_queries++;
        ...
        try {
            ...
            string result_data = SerializeResult(result.get());
            
            string query_hash = HashQuery(query_sql);
            
            return make_unique<QueryResult>(query_hash, result_data, result_size, execution_time);
...
private:
    ...
    size_t EstimateResultSize(MaterializedQueryResult* result) {
        if (!result || result->RowCount() == 0) return 1024;
        
        size_t estimated_size = result->RowCount() * result->ColumnCount() * 32; // 平均每个值32字节
        return max((size_t)1024, estimated_size);
    }
    
    string SerializeResult(MaterializedQueryResult* result) {
        if (!result) return "NULL";
        
        // 简化序列化 - 实际应用中需要更复杂的序列化
...
public:
    LRUCacheStrategy(const string& db_path, size_t max_size = 10, size_t max_mem = 100) 
        : max_cache_size(max_size), max_memory_mb(max_mem) {
    ...
    unique_ptr<QueryResult> ExecuteQuery(const string& query_name, const string& query_sql) override {
        ...
        if (cache_it != cache.end()) {
            stats.cache_hits++;
            stats.total_cache_access_time += 0.5; // 缓存访问时间
...
private:
    unique_ptr<QueryResult> ExecuteQueryDirect(const string& query_name, const string& query_sql) {
        auto start_time = chrono::high_resolution_clock::now();
    ...
    void CacheResult(const string& query_hash, unique_ptr<QueryResult> result) {
        // 检查是否需要淘汰
        EvictIfNeeded();
        
        // 添加到缓存
        auto cache_entry = make_unique<CacheEntry>(query_hash, std::move(result));
...
public:
    TTLCacheStrategy(const string& db_path, chrono::seconds ttl = chrono::seconds(300), size_t max_size = 8) 
...
private:
    ...
    unordered_map<string, unique_ptr<MLCacheEntry>> cache;
    
    size_t max_cache_size;
    size_t current_memory_bytes = 0;
    
    // ML参数
    vector<double> feature_weights = {0.3, 0.25, 0.2, 0.15, 0.1}; // 频率、时间、大小、复杂度、局部性
    double learning_rate = 0.01;
    
    BenchmarkStats stats;
    
public:
    MLCacheStrategy(const string& db_path, size_t max_size = 12) 
...
private:
    bool ShouldCache(const string& query_name, const string& query_sql, QueryResult* result) {
        // 基于ML特征决定是否缓存
    ...
    vector<double> ExtractFeatures(const string& query_name, const string& query_sql, QueryResult* result) {
    ...
    void UpdateMLFeatures(MLCacheEntry* entry) {
        auto now = chrono::steady_clock::now();
    ...
    double PredictNextAccess(MLCacheEntry* entry) {
        if (entry->access_intervals.empty()) return 3600.0; // 默认1小时
    ...
    double CalculateEntryValue(MLCacheEntry* entry) {
        auto now = chrono::steady_clock::now();
    ...
    void CacheResultWithML(const string& query_hash, unique_ptr<QueryResult> result) {
        // 如果缓存已满，智能淘汰
        if (cache.size() >= max_cache_size) {
            EvictLowValueEntry();
        }
        
        auto ml_entry = make_unique<MLCacheEntry>(query_hash, std::move(result));
        current_memory_bytes += ml_entry->result->result_size;
        
        cache[query_hash] = std::move(ml_entry);
        stats.max_cache_size = max(stats.max_cache_size, cache.size());
    }
...
public:
    ...
    void RunBenchmark() {
        ...
        strategies.push_back(make_unique<LRUCacheStrategy>(db_path, 10, 100));
        strategies.push_back(make_unique<TTLCacheStrategy>(db_path, chrono::seconds(300), 8));
        strategies.push_back(make_unique<MLCacheStrategy>(db_path, 12));
...
private:
    vector<pair<string, string>> GenerateWorkload() {
        ...
        // 定义查询访问模式
        // 简单查询 (q01, q06, q14, q17) - 高频访问 50%
        vector<int> simple_queries = {0, 5, 13, 16}; // 对应q01, q06, q14, q17
        
        // 中等查询 - 中频访问 30%
        vector<int> medium_queries = {2, 3, 4, 11, 12, 14, 15}; // q03, q04, q05, q12, q13, q15, q16
        
        // 复杂查询 - 低频访问 20%
        vector<int> complex_queries = {1, 6, 7, 8, 9, 10, 17, 18, 19, 20, 21}; // 其他复杂查询
    ...
    void PrintResults(const vector<BenchmarkStats>& results) {
        ...
        cout << "\n🎯 测试结论:" << endl;
        if (ml_cache && ml_cache->GetHitRate() > 0.3) {
    ...
    void SaveReport(const vector<BenchmarkStats>& results, size_t workload_size) {
        // 确保目录存在
        filesystem::create_directories("cache_test/results");
...
int main() {
    try {
        ...
        if (!filesystem::exists(queries_dir)) {
            cout << "❌ 查询目录不存在: " << queries_dir << endl;
            return 1;
        }
