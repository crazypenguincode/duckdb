...
struct QueryResult {
    ...
    QueryResult(const string& hash, const string& summary, size_t size, double time)
...
public:
    ...
    unique_ptr<QueryResult> ExecuteQuery(const string& query_name, const string& query_sql) override {
        ...
        try {
            ...
            return make_unique<QueryResult>(query_hash, result_summary, result_size, execution_time);
...
private:
    ...
    void UpdateMLFeatures(CacheEntry* entry) {
        auto now = chrono::steady_clock::now();
    ...
    double CalculateEntryValue(CacheEntry* entry) {
        auto now = chrono::steady_clock::now();
