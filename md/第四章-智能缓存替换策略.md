...
策略执行为“策略驱动”的线性流程：系统依据当前配置的 eviction_strategy 选择 TTL_BASED、LRU_BASED 或 ML_BASED 之一进行淘汰；TTL 通过全局 TTL 的过期清理与“最早创建”排序进行淘汰；LRU 通过“最近访问时间”选择最久未访问条目；ML 通过线性权重 + sigmoid 的评分选择低价值条目。 通过策略切换与全局TTL渐进式调优调整替换算法。并设置时间窗抑制频繁切换，以维持稳定性。
