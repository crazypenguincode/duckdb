...
def create_math_element(formula_text):
    """创建 Word 原生数学公式元素 (OMML)"""
    # 简化的 LaTeX 到 OMML 转换
    ...
    def replace_sup(match):
        ...
        return f'<m:sSup><m:e><m:r><m:t>base</m:t></m:r></m:e><m:sup><m:r><m:t>{sup_text}</m:t></m:r></m:sup></m:sSup>'
    ...
    def replace_sub(match):
        ...
        return f'<m:sSub><m:e><m:r><m:t>base</m:t></m:r></m:e><m:sub><m:r><m:t>{sub_text}</m:t></m:r></m:sub></m:sSub>'
    ...
    formula = re.sub(sqrt_pattern, replace_sqrt, formula)
...
def add_math_formula(paragraph, formula_text):
    ...
    try:
        # 尝试创建 OMML 数学公式
        math_element = create_math_element(formula_text)
        ...
        print(f"⚠️  OMML 公式创建失败，使用文本替代: {e}")
        ...
        for pattern, replacement in advanced_replacements.items():
            formula = re.sub(pattern, replacement, formula)
...
def process_chapter(content, doc, chapter_num):
    ...
    while i < len(lines):
        ...
        # 处理普通段落
            ...
            if '$' in line and not line.startswith('$$'):
                ...
                for idx, part in enumerate(parts):
                    if idx % 2 == 1:  # 奇数索引是公式内容
                        success = add_math_formula(p, part)
                        if success:
            ...
            # 处理块级公式 $$...$$
                ...
                p = doc.add_paragraph()
                p.alignment = WD_PARAGRAPH_ALIGNMENT.CENTER
                success = add_math_formula(p, formula)
                if success:
                    omml_success_count += 1
                else:
                    omml_fail_count += 1
    ...
    print(f"📊 章节 {chapter_num} 公式统计:")
    print(f"   - OMML 成功: {omml_success_count}")
    print(f"   - 文本替代: {omml_fail_count}")

def main():
    """主函数"""
    print("🧮 数学公式优化的 Word 文档构建器")
    print("📝 特性：OMML 原生公式 + 强制字体控制")
    ...
    # 保存文档
    output_path = BASE_DIR / "math_optimized_thesis.docx"
