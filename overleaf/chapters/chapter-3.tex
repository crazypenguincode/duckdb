...
\begin{table}[!htb]
    ...
    \begin{tabular}{p{3cm}p{3cm}p{3cm}p{3cm}}
        ...
        缓存阶段 & 缓存内容 & 适用场景 & 性能收益 \\
        \hline
        \textbf{Parser阶段} & 解析后的语法树和CTE映射 & 语法复杂的重复查询 & 避免重复解析开销 \\
        \textbf{Planner阶段} & 逻辑计划和类型信息 & 结构相似的查询变体 & 跳过逻辑规划过程 \\
        \textbf{Optimizer阶段} & 优化后的执行计划 & 参数化查询模板 & 重用优化结果 \\
        \textbf{Executor阶段} & 最终执行结果 & 完全相同的查询 & 直接返回结果 \\
        \hline
    \end{tabular}
\end{table}
...
\If{NOT bloom_filter.MightContain(query_hash)} \Comment{布隆过滤器预筛选检查}
...
\If{cache_entry = NULL} \Comment{缓存条目不存在（假阳性情况）}
\State \Return {ExecuteQuery(query\_string)} \Comment{执行原始查询并返回结果}
\EndIf
\State 
\If{IsExpired(cache_entry)} \Comment{检查缓存条目是否已过期}
...
\begin{table}[!htb]
    ...
    \begin{tabular}{p{3cm}p{3cm}p{3cm}p{3cm}}
        ...
        \textbf{假阳性控制} & 统计监控和参数调整 & 查找效率vs准确性 & 阈值自适应控制 \\
...
\begin{table}[!htb]
    ...
    \begin{tabular}{p{3cm}p{3cm}p{3cm}p{3cm}}
        ...
        \textbf{TTL管理} & 自动过期清理 & 时间窗口滑动 & 保证数据时效性 \\
        \textbf{LRU淘汰} & 容量控制策略 & 最近最少使用 & 优化内存利用率 \\
        \textbf{ML特征} & 智能缓存决策 & 特征工程分析 & 预测访问模式 \\
        \hline
    \end{tabular}
\end{table}
...
\State \Return {query} \Comment{返回标准化后的查询字符串}
\end{algorithmic}
\end{algorithm}

\subsection{CTE细粒度缓存技术}
...
\begin{table}[!htb]
    ...
    \begin{tabular}{p{3cm}p{3cm}p{3cm}p{3cm}}
        ...
        缓存阶段 & 技术特点 & 适用场景 & 性能收益 \\
        \hline
        \textbf{Parser阶段} & 语法树和CTE映射缓存 & 语法复杂的重复查询 & 避免重复解析开销 \\
        \textbf{Planner阶段} & 逻辑计划和类型缓存 & 结构相似的查询变体 & 跳过逻辑规划过程 \\
        \textbf{Optimizer阶段} & 优化计划缓存 & 参数化查询模板 & 重用优化结果 \\
        \textbf{Executor阶段} & 执行结果缓存 & 完全相同的查询 & 直接返回结果 \\
        \hline
    \end{tabular}
\end{table}
...
\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.95\textwidth]{images/3-图3.15 TPC-H查询不同布隆过滤器配置性能对比.png}
	\caption{TPC-H查询不同布隆过滤器配置性能对比}
	\label{fig3_3_15}
\end{figure}

\subsubsection{TPC-H测试关键发现}
