...
# -*- coding: utf-8 -*-
"""
Markdown到LaTeX转换脚本 - 修复版本v5
专门解决表6.7、6.8、6.9等表格标题与内容分离导致的转换问题
"""
...
class MarkdownToLatexConverter:
    def __init__(self, base_dir="/Users/max/src/duckdb"):
        ...
        # 表格标题存储 - 改进版本
        self.table_titles = {}
        self.table_title_positions = {}  # 存储标题在文档中的位置
...
"""
    ...
    def remove_table_titles(self, content: str) -> str:
        """移除独立的表格标题，因为标题已经在LaTeX表格的caption中显示"""
        pattern = r'\*\*表(\d+\.\d+)\s+([^*]+)\*\*\s*\n?'
        content = re.sub(pattern, '', content)
        return content
    
    def extract_table_titles_with_positions(self, content: str) -> str:
        """改进版：提取表格标题并记录位置，但不移除标题"""
        pattern = r'\*\*表(\d+\.\d+)\s+([^*]+)\*\*'
        ...
        for match in matches:
            table_num = match.group(1)  # 如 "6.7"
            title = match.group(2).strip()  # 如 "查询缓存性能实测数据"
            position = match.start()  # 标题在文档中的位置
            
            self.table_titles[table_num] = title
            self.table_title_positions[table_num] = position
            
            print(f"发现表格标题: 表{table_num} - {title} (位置: {position})")
        
        # 暂时不移除原始标题，让表格转换时能找到标题
        return content
    
    def find_nearest_table_title(self, table_position: int, chapter_num: int, content: str) -> str:
        """根据表格在文档中的位置，找到最近的表格标题"""
        # 在表格前后查找标题
        before_text = content[:table_position]
        after_text = content[table_position:table_position + 500]  # 查看表格后500字符
        
        # 查找标题模式
        title_pattern = r'\*\*表(\d+\.\d+)\s+([^*]+)\*\*'
        ...
        if matches_before:
            # 找到最后一个（最近的）标题
            last_match = matches_before[-1]
            table_num = last_match.group(1)
            title = last_match.group(2).strip()
            # 检查是否是当前章节的表格
            if table_num.startswith(f"{chapter_num}."):
                return title
        
        # 如果前面没找到，在表格后查找
        match_after = re.search(title_pattern, after_text)
        if match_after:
            table_num = match_after.group(1)
            title = match_after.group(2).strip()
            if table_num.startswith(f"{chapter_num}."):
                return title
        
        # 如果都没找到，使用预存储的标题
        chapter_titles = {k: v for k, v in self.table_titles.items() 
                         if k.startswith(f"{chapter_num}.")}
        
        if chapter_titles:
            # 返回第一个找到的标题
            return list(chapter_titles.values())[0]
        
        return f"表{chapter_num}.未知"
    ...
    def convert_tables_improved(self, content: str, chapter_num: int) -> str:
        """改进版表格转换 - 解决标题匹配问题"""
        ...
        def replace_table(match):
            ...
            if num_cols <= 2:
                ...
                col_spec = 'p{2.2cm}' * num_cols
            else:
                col_spec = 'p{1.8cm}' * num_cols

            label = f"table{chapter_num}_{table_num}"

            # 改进的标题查找逻辑
            caption = self.find_nearest_table_title(table_position, chapter_num, content)
            if not caption or caption == f"表{chapter_num}.未知":
                caption = f"表{chapter_num}.{table_num}"
            
            print(f"表格 {table_num} 使用标题: {caption}")
...
"""
    ...
    def convert_chapter(self, chapter_num: int) -> bool:
        ...
        # 执行转换步骤 - 改进的顺序
        print(f"  - 提取表格标题...")
        content = self.extract_table_titles_with_positions(content)
        ...
        print(f"  - 转换表格...")
        content = self.convert_tables_improved(content, chapter_num)
        
        print(f"  - 转换参考文献...")
        content = self.convert_references(content)
        
        print(f"  - 移除表格标题...")
        content = self.remove_table_titles(content)
    ...
    def generate_conversion_report(self):
        ...
        report = {
            "conversion_summary": {
                ...
                "table_titles_found": len(self.table_titles)
            },
            "chapter_details": {},
            "files_generated": [],
            "improvements_v5": {
                "table_title_positioning": "改进表格标题匹配，基于位置查找最近标题",
                "enhanced_table_detection": "增强表格检测，支持更宽松的格式匹配",
                "debugging_output": "添加详细的调试输出，便于问题诊断",
                "title_extraction_logging": "记录表格标题提取过程，便于验证"
            },
            "table_titles_extracted": self.table_titles
        }
        ...
        report_file = report_dir / "conversion_report_v5.json"
    ...
    def generate_latex_preamble(self):
        """生成优化的LaTeX导言区配置文件"""
        preamble_content = """% LaTeX导言区配置 - 优化版本v5
% 解决表格标题匹配问题
...
"""
        ...
        preamble_file = self.overleaf_dir / "preamble_v5.tex"
    ...
    def run(self):
        """运行完整的转换流程"""
        print("开始Markdown到LaTeX转换 (v5 - 表格标题匹配修复版)...")
        ...
        print(f"提取的表格标题: {report['conversion_summary']['table_titles_found']}")
        
        print("\nv5版本改进:")
        for key, value in report["improvements_v5"].items():
            print(f"  • {value}")
        ...
        print(f"\n提取的表格标题:")
        for table_num, title in report["table_titles_extracted"].items():
            print(f"  表{table_num}: {title}")
        ...
        # 使用建议
        print("\n📋 v5版本特别说明:")
        print("1. 修复了表6.7、6.8、6.9等表格标题匹配问题")
        print("2. 改进了表格标题提取逻辑，基于位置查找最近标题")
        print("3. 增加了详细的调试输出，便于问题诊断")
        print("4. 支持更宽松的表格格式匹配")
        print("5. 如果仍有问题，请检查上面的'提取的表格标题'列表")
